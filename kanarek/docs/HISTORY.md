...
- **Grupowanie playera po `group-title`**: płaska lista stacji w `PlayerActivity` dostaje
  ...
  zamiast nieskończącego się scrolla. Model/`M3uCodec` bez zmian — `groupTitle` już był
...
## Zakładki TV/Radio + now playing + fallback logo (feat/player-tv-radio-split)

- Lista stacji dzieli się na dwie zakładki z ikonami: **TV** (telewizor — strumienie wideo,
  w tym nieoznaczone/`UNKNOWN` jako "możliwe wideo") i **Radio** (radyjko — audio).
  Zakładki pojawiają się tylko, gdy lista faktycznie ma oba rodzaje; czysto radiowa albo
  czysto telewizyjna lista zostaje płaska. Stary trójprzyciskowy filtr (Wszystko/TV/Radio)
  wyleciał.
- Radio dostało panel **now playing** w miejscu, gdzie TV ma obraz: duże logo stacji, nazwa,
  grupa i — gdy strumień nadaje metadane — aktualnie grany utwór (marquee przy długich
  tytułach). Źródło: ICY `StreamTitle` (ExoPlayer prosi o `Icy-MetaData: 1` domyślnie dla
  strumieni progresywnych) oraz ID3 `TIT2` (HLS audio), złożone w `PlayerUiState.nowPlaying`
  w `PlayerService.onMetadata`. Utwór trafia też do dolnego paska playera i do subtitle
  widgetu playera (zamiast grupy, gdy jest).
- **Fallback logo**: stacja bez `tvg-logo` (np. z domyślnych playlist) pokazuje favikonę
  hosta strumienia — najpierw Google s2 (zawsze PNG, Coil dekoduje), potem DuckDuckGo,
  na końcu wbudowany glif. Łańcuch prób w `StationLogo` (indeks kandydata podbijany
  w `onError`).
