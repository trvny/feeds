...
- `feedseek/feeds/`: compatibility mirror for raw URLs embedded in released Kanarek clients; generated by `tools/sync_legacy_feed_paths.py`, never edit it by hand. It can be dropped, together with that script and its `update-feeds.yml` step, once no installed client still reads `.../feedseek/feeds/` — which means after a Kanarek release built from `trvny/kanarek` has replaced Kanarek 1.0.1 on every device that runs it.
...
- The Android app and its Worker live in `trvny/kanarek`; nothing about them belongs here any more. The one remaining tie is `feedseek/feeds/`.
