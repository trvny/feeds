...
SOURCES = [
    ...
    (
        "Sony PL Wallpapers",
        "https://community.sony.pl/sonyeu1/rss/board?board.id=wallpaper_world",
        30,
    ),
]
...
def scrape_sony_group(known_links):
    ...
    for item in soup.find_all("item")[:60]:
        try:
            ...
            link = (
                urljoin(GROUP_RSS_URL, link_el.get_text(strip=True)) if link_el else ""
            )
            ...
            entries.append(
                {
                    ...
                    "link": link,
                    "date": (
                        parse_date(item.find("pubDate").get_text(strip=True))
